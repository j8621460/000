...
class HomeScreenFragment : Fragment(R.layout.fragment_home_screen) {
    ...
    private fun initializeClickListeners() {
        ...
        b.fhsCardWgToggle?.setOnCheckedChangeListener { btn, isChecked ->
            // only react to user interaction, not to programmatic state sync
            if (!btn.isPressed) return@setOnCheckedChangeListener
            Logger.v(LOG_TAG_UI, "$TAG: wireguard toggle changed to $isChecked")
            toggleWireguard(isChecked)
            logEvent(
                EventType.UI_TOGGLE,
                "HomeScreen: WireGuard toggle clicked",
                "WireGuard toggled to $isChecked from HomeScreenFragment"
            )
        }
    ...
    private fun syncWireguardToggle() {
        if (view == null || !isAdded) return

        b.fhsCardWgToggle?.isChecked = appConfig.isWireGuardEnabled()
    }

    // enables/disables wireguard right from the home screen proxy card
    private fun toggleWireguard(enable: Boolean) {
        io {
            if (enable) {
                ...
                if (mappings.isEmpty()) {
                    uiCtx {
                        ...
                        b.fhsCardWgToggle?.isChecked = false
                ...
                if (!WireguardManager.canEnableProxy()) {
                    uiCtx {
                        ...
                        showToastUiCentered(
                            requireContext(),
                            getString(
                                R.string.settings_socks5_disabled_error,
                                getString(R.string.lbl_proxy)
                            ),
                            Toast.LENGTH_SHORT
                        )
                        b.fhsCardWgToggle?.isChecked = false
                ...
                val active = mappings.filter { it.isActive }
                when {
                    active.isNotEmpty() -> {
                        // configs already marked active, just select wireguard as the proxy
                        appConfig.addProxy(
                            AppConfig.ProxyType.WIREGUARD,
                            AppConfig.ProxyProvider.WIREGUARD
                        )
                    }
                    mappings.size == 1 -> {
                        WireguardManager.enableConfig(mappings.first())
                    }
                    else -> {
                        // multiple configs, let the user pick which one to enable
                        uiCtx {
                            if (!isAdded) return@uiCtx
                            b.fhsCardWgToggle?.isChecked = false
                            startActivity(ScreenType.PROXY_WIREGUARD)
                ...
                if (!WireguardManager.canDisableAllActiveConfigs()) {
                    uiCtx {
                        ...
                        b.fhsCardWgToggle?.isChecked = true
                    }
                    return@io
                }
